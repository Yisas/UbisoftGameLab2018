...
public class NGSS_Directional : MonoBehaviour
{
    ...
    */
    ...
    void Update()
    {
        //if (BIAS_FADE) { Shader.EnableKeyword("NGSS_USE_BIAS_FADE_DIR"); Shader.SetGlobalFloat("NGSS_BIAS_FADE_DIR", BIAS_FADE_VALUE * 0.001f); } else { Shader.DisableKeyword("NGSS_USE_BIAS_FADE_DIR"); }
        if (NOISE_STATIC) { Shader.EnableKeyword("NGSS_NOISE_STATIC_DIR"); } else { Shader.DisableKeyword("NGSS_NOISE_STATIC_DIR"); }
        if (CASCADES_BLENDING && QualitySettings.shadowCascades > 1) { Shader.EnableKeyword("NGSS_USE_CASCADE_BLENDING"); Shader.SetGlobalFloat("NGSS_CASCADE_BLEND_DISTANCE", CASCADES_BLENDING_VALUE * 0.125f); } else { Shader.DisableKeyword("NGSS_USE_CASCADE_BLENDING"); }
